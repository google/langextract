#!/usr/bin/env python3
"""Test script for doubao provider (Step 5 checklist)."""
...
try:
    from langextract_doubao import doubaoLanguageModel
except ImportError:
...
PROVIDER_CLS_NAME = "doubaoLanguageModel"
PATTERNS = ['^doubao']
...
print("Testing doubao Provider - Step 5 Checklist:")
print("-" * 50)
...
try:
    ...
    provider = doubaoLanguageModel(model_id=model_id)
...
try:
    ...
    schema = doubaoSchema.from_examples(examples)
    print(f"   ✓ Schema created (keys={list(schema.schema_dict.keys())})")

    schema_class = doubaoLanguageModel.get_schema_class()
    print(f"   ✓ Provider schema class: {schema_class.__name__}")

    provider = doubaoLanguageModel(model_id=_example_id(PATTERNS[0]) if PATTERNS else "test-model")
